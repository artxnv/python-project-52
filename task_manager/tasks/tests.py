...
class TasksCrudTestCase(TestCase):
    ...
    def test_create_task(self):
        ...
        self.assertContains(
            response, _('Create task'), status_code=200
        )
        ...
        self.assertContains(
            response, _('Task is successfully created'), status_code=200
        )
        self.assertTrue(
            Task.objects.filter(name=self.test_tasks["new"]["name"]).exists()
        )
    ...
    def test_update_task(self):
        ...
        self.assertContains(
            response, _('Task is successfully changed'), status_code=200
        )
    ...
    def test_delete_task_successfully(self):
        ...
        response = self.client.post(request_url, follow=True)
        self.assertContains(
            response, _('Task is successfully deleted'), status_code=200
        )
